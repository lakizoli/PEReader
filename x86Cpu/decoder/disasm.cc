...
char* disasm(char *disbufptr, const bxInstruction_c *i, bx_address cs_base, bx_address rip)
{
  ...
  unsigned opname_len = (unsigned) strlen(opname);
...
char* disasm(BX_CPU_C* cpu, const Bit8u *opcode, bool is_32, bool is_64, char *disbufptr, bxInstruction_c *i, bx_address cs_base, bx_address rip)
{
...
#if BX_SUPPORT_X86_64
  if (is_64)
    ret = fetchDecode64(opcode, fetchModeMask, cpu->is_cpu_extension_supported(BX_ISA_ALT_MOV_CR8), i, 16);
  else
#endif
    ret = fetchDecode32(opcode, fetchModeMask, cpu->is_cpu_extension_supported(BX_ISA_ALT_MOV_CR8), i, 16);
