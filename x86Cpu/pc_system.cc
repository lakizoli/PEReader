...
void bx_pc_system_c::set_HRQ(BX_CPU_C* cpu, bx_bool val)
{
  HRQ = val;
  if (val) {
    cpu->async_event = 1;
...
void bx_pc_system_c::raise_INTR(BX_CPU_C* cpu)
{
...
void bx_pc_system_c::clear_INTR(BX_CPU_C* cpu)
{
  //if (bx_dbg.interrupts)
...
void bx_pc_system_c::set_enable_a20(BX_CPU_C* cpu, bx_bool value)
{
#if BX_SUPPORT_A20
  ...
  if (old_enable_a20 != enable_a20) MemoryMappingChanged(cpu);
#else
...
void bx_pc_system_c::MemoryMappingChanged(BX_CPU_C* cpu)
{
  //for (unsigned i=0; i<BX_SMP_PROCESSORS; i++)
    cpu->TLB_flush();
}

void bx_pc_system_c::invlpg(BX_CPU_C* cpu, bx_address addr)
{
  //for (unsigned i=0; i<BX_SMP_PROCESSORS; i++)
    cpu->TLB_invlpg(addr);
}

int bx_pc_system_c::Reset(BX_CPU_C* cpu, unsigned type)
{
...
{
  return (Bit64u) (((double)(Bit64s)time_ticks(this)) / m_ips);
}
