...
void flushICaches(BX_CPU_C* cpu)
{
  //for (unsigned i=0; i<BX_SMP_PROCESSORS; i++) {
    cpu->iCache.flushICacheEntries();
...
void handleSMC(BX_CPU_C* cpu, bx_phy_address pAddr, Bit32u mask)
{
  INC_SMC_STAT(smc);

  //for (unsigned i=0; i<BX_SMP_PROCESSORS; i++) {
    cpu->async_event |= BX_ASYNC_EVENT_STOP_TRACE;
    cpu->iCache.handleSMC(pAddr, mask);
  //}
}
