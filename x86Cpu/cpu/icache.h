...
extern void handleSMC(BX_CPU_C* cpu, bx_phy_address pAddr, Bit32u mask);
...
public:
 ...
 ~bxPageWriteStampTable() { delete [] fineGranularityMapping; }
  ...
  BX_CPP_INLINE void decWriteStamp(BX_CPU_C* cpu, bx_phy_address pAddr)
  {
    Bit32u index = hash(pAddr);

    if (fineGranularityMapping[index]) {
      handleSMC(cpu, pAddr, 0xffffffff); // one of the CPUs might be running trace from this page
      fineGranularityMapping[index] = 0;
    }
  }

  // assumption: write does not split 4K page
  BX_CPP_INLINE void decWriteStamp(BX_CPU_C* cpu, bx_phy_address pAddr, unsigned len)
  {
    Bit32u index = hash(pAddr);
...
//  return ((pAddr + (pAddr << 2) + (pAddr>>6)) & (BxICacheEntries-1)) ^ fetchModeMask;
  ...
  BX_CPP_INLINE void flushICacheEntries();
...
extern void flushICaches(BX_CPU_C* cpu);
