...
#ifdef CPU_HAS_DEVICES
#	include "iodev/iodev.h"
#endif //CPU_HAS_DEVICES
...
int apic_bus_deliver_interrupt(BX_CPU_C* cpu, Bit8u vector, apic_dest_t dest, Bit8u delivery_mode, bx_bool logical_dest, bx_bool level, bx_bool trig_mode)
{
  if(delivery_mode == APIC_DM_LOWPRI)
  {
     if(! logical_dest) {
       ...
       return apic_bus_deliver_lowest_priority(cpu, vector, dest, trig_mode, 0);
     }
  }
  ...
  if(! logical_dest) {
    ...
    if((dest & cpu->apic_id_mask) == cpu->apic_id_mask) {
       return apic_bus_broadcast_interrupt(cpu, vector, delivery_mode, trig_mode, cpu->apic_id_mask);
    }
    else {
       // the destination is single agent
       for (unsigned i=0;i<BX_NUM_LOCAL_APICS;i++)
       {
         if(cpu->lapic.get_id() == dest) {
			 cpu->lapic.deliver(vector, delivery_mode, trig_mode);
           return 1;
         }
       }
    ...
    for (int i=0; i<BX_NUM_LOCAL_APICS; i++) {
      if(cpu->lapic.match_logical_addr(dest)) {
		  cpu->lapic.deliver(vector, delivery_mode, trig_mode);
...
int apic_bus_deliver_lowest_priority(BX_CPU_C* cpu, Bit8u vector, apic_dest_t dest, bx_bool trig_mode, bx_bool broadcast)
{
  int i;

  if (!cpu->lapic.is_xapic()) {
    // search for if focus processor exists
    for (i=0; i<BX_NUM_LOCAL_APICS; i++) {
      if(cpu->lapic.is_focus(vector)) {
		  cpu->lapic.deliver(vector, APIC_DM_LOWPRI, trig_mode);
        return 1;
      }
    }
  }
  ...
  for (i=0; i<BX_NUM_LOCAL_APICS; i++) {
    if(broadcast || cpu->lapic.match_logical_addr(dest)) {
      if (cpu->lapic.is_xapic())
        priority = cpu->lapic.get_tpr();
      else
        priority = cpu->lapic.get_apr();
...
int apic_bus_broadcast_interrupt(BX_CPU_C* cpu, Bit8u vector, Bit8u delivery_mode, bx_bool trig_mode, int exclude_cpu)
{
  if(delivery_mode == APIC_DM_LOWPRI)
  {
    return apic_bus_deliver_lowest_priority(cpu, vector, 0 /* doesn't matter */, trig_mode, 1);
  }
...
BOCHSAPI_MSVCONLY void apic_bus_deliver_smi(BX_CPU_C* cpu)
{
  cpu->deliver_SMI();
}

void apic_bus_broadcast_smi(BX_CPU_C* cpu)
{
...
bx_local_apic_c::bx_local_apic_c(BX_CPU_C *mycpu, bx_pc_system_c* sys, bx_bool simulate_xapic, unsigned id)
  : base_addr(BX_LAPIC_BASE_ADDR), cpu(mycpu)
{
  ...
  mwaitx_timer_handle = sys->register_timer_ticks(this,
...
void bx_local_apic_c::reset(unsigned type)
{
  ...
  if(timer_active) {
    cpu->sys->deactivate_timer(timer_handle);
    timer_active = 0;
  }

#if BX_SUPPORT_VMX >= 2
  if(vmx_timer_active) {
	  cpu->sys->deactivate_timer(vmx_timer_handle);
    vmx_timer_active = 0;
  }
#endif
  ...
  if(mwaitx_timer_active) {
	  cpu->sys->deactivate_timer(mwaitx_timer_handle);
    mwaitx_timer_active = 0;
  }
#endif
...
void bx_local_apic_c::send_ipi(apic_dest_t dest, Bit32u lo_cmd)
{
  ...
  case 0:  // no shorthand, use real destination value
    accepted = apic_bus_deliver_interrupt(cpu, vector, dest, delivery_mode, logical_dest, level, trig_mode);
    break;
  case 1:  // self
  ...
  case 2:  // all including self
    accepted = apic_bus_broadcast_interrupt(cpu, vector, delivery_mode, trig_mode, cpu->apic_id_mask);
    break;
  case 3:  // all but self
    accepted = apic_bus_broadcast_interrupt(cpu, vector, delivery_mode, trig_mode, get_id());
    break;
  default:
...
void bx_local_apic_c::periodic(void)
{
  ...
  if(timervec & 0x20000) {
    ...
    timer_active = 1;
    ticksInitial = bx_pc_system_c::time_ticks(cpu->sys); // timer value when it started to count
    BX_DEBUG(("local apic timer(periodic) triggered int, reset counter to 0x%08x", timer_current));
    cpu->sys->activate_timer_ticks(timer_handle,
...
Bit32u bx_local_apic_c::get_current_timer_count(void)
{
  ...
  if(timer_active==0) {
    ...
    Bit64u delta64 = (bx_pc_system_c::time_ticks(cpu->sys) - ticksInitial) / timer_divide_factor;
...
void bx_local_apic_c::set_tsc_deadline(Bit64u deadline)
{
  ...
  if (deadline != 0) {
    ...
    Bit64u currtime = bx_pc_system_c::time_ticks(cpu->sys);
    timer_active = 1;
	cpu->sys->activate_timer_ticks(timer_handle, (deadline > currtime) ? (deadline - currtime) : 1 , 0);
  }
}
...
{
  Bit32u diff = (Bit32u) ((bx_pc_system_c::time_ticks(cpu->sys) >> vmx_preemption_timer_rate) - (vmx_preemption_timer_initial >> vmx_preemption_timer_rate));
...
void bx_local_apic_c::set_vmx_preemption_timer(Bit32u value)
{
  ...
  vmx_preemption_timer_initial = bx_pc_system_c::time_ticks(cpu->sys);
  ...
  cpu->sys->activate_timer_ticks(vmx_timer_handle, vmx_preemption_timer_fire - vmx_preemption_timer_initial, 0);
  vmx_timer_active = 1;
}
...
void bx_local_apic_c::set_mwaitx_timer(Bit32u value)
{
  ...
  cpu->sys->activate_timer_ticks(mwaitx_timer_active, value, 0);
