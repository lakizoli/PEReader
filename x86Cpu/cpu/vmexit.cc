...
void BX_CPU_C::VMexit_PAUSE(void)
{
  ...
  if (SECONDARY_VMEXEC_CONTROL(VMX_VM_EXEC_CTRL3_PAUSE_LOOP_VMEXIT) && CPL == 0) {
    ...
    Bit64u currtime = bx_pc_system_c::time_ticks(sys);
  ...
  if (! VMEXIT(VMX_VM_EXEC_CTRL2_MSR_BITMAPS)) vmexit = 1;
    ...
    if (msr >= BX_VMX_HI_MSR_START) {
       if (msr > BX_VMX_HI_MSR_END) vmexit = 1;
         ...
         access_read_physical(mem, pAddr, 1, &field);
  ...
  if (VMEXIT(VMX_VM_EXEC_CTRL2_IO_BITMAPS)) {
     ...
     if ((port + len) > 0x10000) vmexit = 1;
        ...
        if ((port & 0x7fff) + len > 0x8000) {
          ...
          access_read_physical(mem, pAddr, 1, &bitmap[0]);
          ...
          access_read_physical(mem, pAddr, 1, &bitmap[1]);
...
bx_bool BX_CPP_AttrRegparmN(1) BX_CPU_C::Vmexit_Vmread(bxInstruction_c *i)
{
  ...
  access_read_physical(mem, pAddr, 1, &bitmap);
...
void BX_CPU_C::Virtualization_Exception(Bit64u qualification, Bit64u guest_physical, Bit64u guest_linear)
{
  ...
  Bit32u magic;
  access_read_physical(mem, vm->ve_info_addr + 4, 4, &magic);
...
void BX_CPU_C::vmx_page_modification_logging(Bit64u guest_paddr, unsigned dirty_update)
{
  ...
  if (dirty_update) {
    ...
    access_write_physical(mem, pAddr, 8, &guest_paddr);
