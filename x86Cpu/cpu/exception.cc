...
void BX_CPU_C::protected_mode_int(Bit8u vector, unsigned soft_int, bx_bool push_error, Bit16u error_code)
{
  ...
  case BX_TASK_GATE:
    ...
    task_switch(mem, 0, &tss_selector, &tss_descriptor,
...
#ifdef CPU_HAS_PARAM_STATE_HOOK
	  bool have_to_reset_on_triple_fault = SIM->get_param_bool (BXPN_RESET_ON_TRIPLE_FAULT)->get ();
#else //CPU_HAS_PARAM_STATE_HOOK
	  bool have_to_reset_on_triple_fault = false;
#endif //CPU_HAS_PARAM_STATE_HOOK
	  if (have_to_reset_on_triple_fault) {
		BX_ERROR(("exception(): 3rd (%d) exception with no resolution, shutdown status is %02xh, resetting", vector, DEV_cmos_get_reg(0x0f)));
        sys->Reset(this, BX_RESET_HARDWARE);
      }
      else {
