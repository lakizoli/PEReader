...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::RSM(bxInstruction_c *i)
{
  ...
  for(n=0;n<SMM_SAVE_STATE_MAP_SIZE;n++) {
    base -= 4;
    access_read_physical(mem, base, 4, &saved_state[n]);
  ...
  if (! smram_restore_state(mem, saved_state)) {
...
void BX_CPU_C::enter_system_management_mode(BX_MEM_C* mem)
{
  ...
  for(n=0;n<SMM_SAVE_STATE_MAP_SIZE;n++) {
    base -= 4;
    access_write_physical(mem, base, 4, &saved_state[n]);
...
bx_bool BX_CPU_C::smram_restore_state(BX_MEM_C* mem, const Bit32u *saved_state)
{
  ...
  return resume_from_system_management_mode(mem, &smm_state);
}
...
bx_bool BX_CPU_C::resume_from_system_management_mode(BX_MEM_C* mem, BX_SMM_State *smm_state)
{
  ...
  if (BX_CPU_THIS_PTR cr0.get_PG() && BX_CPU_THIS_PTR cr4.get_PAE() && !long_mode()) {
    if (! CheckPDPTR(mem, smm_state->cr3)) {
