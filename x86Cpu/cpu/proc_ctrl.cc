...
{
  Bit64u tsc = bx_pc_system_c::time_ticks(sys) - BX_CPU_THIS_PTR tsc_last_reset;
...
void BX_CPU_C::set_TSC(Bit64u newval)
{
  ...
  // will return newval
  BX_CPU_THIS_PTR tsc_last_reset = bx_pc_system_c::time_ticks(sys) - newval;
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::MWAIT(bxInstruction_c *i)
{
  ...
  static bx_bool mwait_is_nop = true;
#endif //CPU_HAS_PARAM_STATE_HOOK
  if (mwait_is_nop) {
    BX_NEXT_TRACE(i);
  }
