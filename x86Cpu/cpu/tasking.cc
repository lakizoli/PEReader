/////////////////////////////////////////////////////////////////////////
// $Id: tasking.cc 12836 2015-09-22 20:10:22Z sshwarts $
/////////////////////////////////////////////////////////////////////////
//
...
void BX_CPU_C::task_switch(BX_MEM_C* mem, bxInstruction_c *i, bx_selector_t *tss_selector,
                 ...
                 Bit32u dword1, Bit32u dword2, bx_bool push_error, Bit32u error_code)
{
  ...
  {
    translate_linear(mem, BX_TLB_ENTRY_OF(nbase32,               0), nbase32,               0, BX_READ); // old TSS
    translate_linear(mem, BX_TLB_ENTRY_OF(nbase32 + new_TSS_max, 0), nbase32 + new_TSS_max, 0, BX_READ);
    ...
    {
      translate_linear(mem, BX_TLB_ENTRY_OF(nbase32,     0), nbase32,     0, BX_WRITE);
      translate_linear(mem, BX_TLB_ENTRY_OF(nbase32 + 1, 0), nbase32 + 1, 0, BX_WRITE);
    }
  }
  ...
  if (source == BX_TASK_FROM_JUMP || source == BX_TASK_FROM_IRET) {
    ...
    access_read_linear(mem, laddr, 4, 0, BX_RW, 0x0, &temp32);
    temp32 &= ~0x200;
    access_write_linear(mem, laddr, 4, 0, 0x0, &temp32);
  }
  ...
  if (BX_CPU_THIS_PTR tr.cache.type <= 3) {
    ...
    if (BX_CPU_THIS_PTR cr0.get_PG()) {
      ...
      translate_linear(mem, BX_TLB_ENTRY_OF(start, 0), start, 0, BX_WRITE);
      translate_linear(mem, BX_TLB_ENTRY_OF(end, 0),   end,   0, BX_WRITE);
    }
  ...
  if (source != BX_TASK_FROM_IRET)
  {
    ...
    access_read_linear(mem, laddr, 4, 0, BX_RW, 0x0, &dword2);
    dword2 |= 0x200;
    access_write_linear(mem, laddr, 4, 0, 0x0, &dword2);
  }
