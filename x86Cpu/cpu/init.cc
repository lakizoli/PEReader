...
BX_CPU_C::BX_CPU_C(BX_MEM_C* mem, bx_pc_system_c* sys, unsigned id): mem(mem), sys(sys), bx_cpuid(id)
#if BX_CPU_LEVEL >= 4
   , cpuid(NULL)
#endif
#if BX_SUPPORT_APIC
   ,apic_id_mask (0xFF) //simulate_xapic ? 0xFF : 0xF;
   ,lapic (this, sys, true, id) //simulate_xapic = true
#endif
{
