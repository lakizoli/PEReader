...
#define BX_HANDLERS_CHAINING_MAX_DEPTH 1000
  ...
  if(delta >= bx_pc_system_c::getNumCpuTicksLeftNextEvent(sys)) {
    linkDepth = 0;
    return;
  }
...
void BX_CPU_C::prefetch(void)
{
  ...
  if ((tlbEntry->lpf == lpf) && (tlbEntry->accessBits & (0x10 << USER_PL)) != 0) {
    ...
    bx_phy_address pAddr = translate_linear(mem, tlbEntry, laddr, USER_PL, BX_EXECUTE);
  ...
  if (fetchPtr) {
    ...
    BX_CPU_THIS_PTR eipFetchPtr = (const Bit8u*) getHostMemAddr(mem, BX_CPU_THIS_PTR pAddrFetchPage, BX_EXECUTE);

    // Sanity checks
    if (! BX_CPU_THIS_PTR eipFetchPtr) {
      bx_phy_address pAddr = BX_CPU_THIS_PTR pAddrFetchPage + pageOffset;
      if (pAddr >= mem->get_memory_len()) {
