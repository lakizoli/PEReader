...
bx_address bx_asize_mask[] = {
  0xffffull,                      // as16 (asize = '00)
  0xffffffffull,                  // as32 (asize = '01)
#if BX_SUPPORT_X86_64
  0xffffffffffffffffull,		  // as64 (asize = '10)
  0xffffffffffffffffull			  // as64 (asize = '11)
#endif
};
...
BX_CPU_C::system_read_byte(bx_address laddr)
{
  ...
  if (access_read_linear(mem, laddr, 1, 0, BX_READ, 0x0, (void *) &data) < 0)
    exception(BX_GP_EXCEPTION, 0);
...
BX_CPU_C::system_read_word(bx_address laddr)
{
  ...
  if (access_read_linear(mem, laddr, 2, 0, BX_READ, 0x0, (void *) &data) < 0)
    exception(BX_GP_EXCEPTION, 0);
...
BX_CPU_C::system_read_dword(bx_address laddr)
{
  ...
  if (access_read_linear(mem, laddr, 4, 0, BX_READ, 0x0, (void *) &data) < 0)
    exception(BX_GP_EXCEPTION, 0);
...
BX_CPU_C::system_read_qword(bx_address laddr)
{
  ...
  if (access_read_linear(mem, laddr, 8, 0, BX_READ, 0x0, (void *) &data) < 0)
    exception(BX_GP_EXCEPTION, 0);
...
BX_CPU_C::system_write_byte(bx_address laddr, Bit8u data)
{
  ...
  if (access_write_linear(mem, laddr, 1, 0, 0x0, (void *) &data) < 0)
    exception(BX_GP_EXCEPTION, 0);
}
...
BX_CPU_C::system_write_word(bx_address laddr, Bit16u data)
{
  ...
  if (access_write_linear(mem, laddr, 2, 0, 0x0, (void *) &data) < 0)
    exception(BX_GP_EXCEPTION, 0);
}
...
BX_CPU_C::system_write_dword(bx_address laddr, Bit32u data)
{
  ...
  if (access_write_linear(mem, laddr, 4, 0, 0x0, (void *) &data) < 0)
    exception(BX_GP_EXCEPTION, 0);
}
...
BX_CPU_C::v2h_write_byte(bx_address laddr, bx_bool user)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    if (isWriteOK(tlbEntry, user)) {
      ...
      Bit32u pageOffset = PAGE_OFFSET(laddr);
      Bit8u *hostAddr = (Bit8u*) (hostPageAddr | pageOffset);
      pageWriteStampTable.decWriteStamp(this, tlbEntry->ppf);
      return hostAddr;
    }
  }
