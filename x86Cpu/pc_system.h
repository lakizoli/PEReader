...
public:
  ...
  static BX_CPP_INLINE void tick1(bx_pc_system_c* sys) {
    if (--sys->currCountdown == 0) {
      sys->countdownEvent();
    }
  }
  static BX_CPP_INLINE void tickn(bx_pc_system_c* sys, Bit32u n) {
    while (n >= sys->currCountdown) {
      n -= sys->currCountdown;
	  sys->currCountdown = 0;
	  sys->countdownEvent();
      // sys->currCountdown is adjusted to new value by countdownevent().
    }
  ...
  static BX_CPP_INLINE Bit64u time_ticks(bx_pc_system_c* sys) {
    return sys->ticksTotal +
      Bit64u(sys->currCountdownPeriod - sys->currCountdown);
  }

  static BX_CPP_INLINE Bit32u  getNumCpuTicksLeftNextEvent(bx_pc_system_c* sys) {
    return sys->currCountdown;
  }
#if BX_DEBUGGER
  ...
  void set_HRQ(BX_CPU_C* cpu, bx_bool val);  // set the Hold ReQuest line

  void raise_INTR(BX_CPU_C* cpu);
  void clear_INTR(BX_CPU_C* cpu);

  // Cpu and System Reset
  int Reset(BX_CPU_C* cpu, unsigned type);
  Bit8u  IAC(void);
...
#endif //CPU_HAS_INOUT
  void    set_enable_a20(BX_CPU_C* cpu, bx_bool value);
  bx_bool get_enable_a20(void);
  void    MemoryMappingChanged(BX_CPU_C* cpu); // flush TLB in all CPUs
  void    invlpg(BX_CPU_C* cpu, bx_address addr);    // flush TLB page in all CPUs
  void    exit(void);
