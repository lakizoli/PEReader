...
public:
	...
	BX_MEM_SMF Bit8u*  get_vector (BX_CPU_C *cpu, bx_phy_address addr);
	...
	BX_MEM_SMF void    dmaWritePhysicalPage (BX_CPU_C *cpu, bx_phy_address addr, unsigned len, Bit8u *data);

	BX_MEM_SMF void    load_ROM (const char *path, bx_phy_address romaddress, Bit8u type);
	BX_MEM_SMF void    load_RAM (BX_CPU_C *cpu, const char *path, bx_phy_address romaddress);
	...
	BX_MEM_SMF void allocate_block (BX_CPU_C *cpu, Bit32u index);
	BX_MEM_SMF Bit8u* alloc_vector_aligned (Bit32u bytes, Bit32u alignment);

#if BX_SUPPORT_MONITOR_MWAIT
	BX_MEM_SMF bx_bool is_monitor (BX_CPU_C *cpu, bx_phy_address begin_addr, unsigned len);
	BX_MEM_SMF void    check_monitor (BX_CPU_C *cpu, bx_phy_address addr, unsigned len);
#endif
...
BX_CPP_INLINE Bit8u* BX_MEM_C::get_vector (BX_CPU_C *cpu, bx_phy_address addr) {
