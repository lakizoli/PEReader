...
void BX_MEM_C::init_memory (Bit64u guest, Bit64u host) {
	...
	BX_MEM_THIS vector = alloc_vector_aligned (Bit32u (host + BIOSROMSZ + EXROMSIZE + 4096), BX_MEM_VECTOR_ALIGN);
...
void BX_MEM_C::allocate_block (BX_CPU_C *cpu, Bit32u block) {
...
void BX_MEM_C::load_ROM (const char *path, bx_phy_address romaddress, Bit8u type) {
	...
	if (type == 0) {
		if (romaddress > 0) {
			...
			romaddress = (bx_phy_address) (- (signed long)size);
		}
...
void BX_MEM_C::load_RAM (BX_CPU_C *cpu, const char *path, bx_phy_address ramaddress) {
	struct stat stat_buf;
	int fd, ret;
	Bit32u size, offset;
	...
	offset = (Bit32u) ramaddress;
	while (size > 0) {
		ret = read (fd, (bx_ptr_t) BX_MEM_THIS get_vector (cpu, offset), size);
		if (ret <= 0) {
...
Bit8u *BX_MEM_C::getHostMemAddr (BX_CPU_C *cpu, bx_phy_address addr, unsigned rw) {
	...
	if ((cpu != NULL) && (rw == BX_EXECUTE)) {
		...
		if ((a20addr >= 0x000a0000 && a20addr < 0x000c0000) && (BX_MEM_THIS smram_available)) {
			if (BX_MEM_THIS smram_enable || cpu->smm_mode ())
				return BX_MEM_THIS get_vector (cpu, a20addr);
		}
	}

#if BX_SUPPORT_MONITOR_MWAIT
	if (write && BX_MEM_THIS is_monitor (cpu, a20addr & ~((bx_phy_address) (0xfff)), 0xfff)) {
...
bx_bool BX_MEM_C::is_monitor (BX_CPU_C *cpu, bx_phy_address begin_addr, unsigned len) {
	//for (int i = 0; i<BX_SMP_PROCESSORS; i++) {
		if (cpu->is_monitor (begin_addr, len))
			return 1;
	//}
...
void BX_MEM_C::check_monitor (BX_CPU_C *cpu, bx_phy_address begin_addr, unsigned len) {
	//for (int i = 0; i<BX_SMP_PROCESSORS; i++) {
...
#if BX_SUPPORT_MONITOR_MWAIT
	BX_MEM_THIS check_monitor (cpu, a20addr, len);
#endif
...
mem_write:
	...
	if (a20addr < BX_MEM_THIS len && !is_bios) {
		...
		if (a20addr < 0x000a0000 || a20addr >= 0x00100000) {
			if (len == 8) {
				pageWriteStampTable.decWriteStamp (cpu, a20addr, 8);
				WriteHostQWordToLittleEndian (BX_MEM_THIS get_vector (cpu, a20addr), *(Bit64u*) data);
				return;
			}
			if (len == 4) {
				pageWriteStampTable.decWriteStamp (cpu, a20addr, 4);
				WriteHostDWordToLittleEndian (BX_MEM_THIS get_vector (cpu, a20addr), *(Bit32u*) data);
				return;
			}
			if (len == 2) {
				pageWriteStampTable.decWriteStamp (cpu, a20addr, 2);
				WriteHostWordToLittleEndian (BX_MEM_THIS get_vector (cpu, a20addr), *(Bit16u*) data);
				return;
			}
			if (len == 1) {
				pageWriteStampTable.decWriteStamp (cpu, a20addr, 1);
				*(BX_MEM_THIS get_vector (cpu, a20addr)) = *(Bit8u *) data;
				return;
			}
			// len == other, just fall thru to special cases handling
		}

		pageWriteStampTable.decWriteStamp (cpu, a20addr);
		...
		if (a20addr < 0x000a0000 || a20addr >= 0x00100000) {
			...
			while (1) {
				*(BX_MEM_THIS get_vector (cpu, a20addr)) = *data_ptr;
				if (len == 1) return;
				len--;
				a20addr++;
#ifdef BX_LITTLE_ENDIAN
...
mem_read:
	...
	if (a20addr < BX_MEM_THIS len && !is_bios) {
		...
		if (a20addr < 0x000a0000 || a20addr >= 0x00100000) {
			if (len == 8) {
				ReadHostQWordFromLittleEndian (BX_MEM_THIS get_vector (cpu, a20addr), *(Bit64u*) data);
				return;
			}
			if (len == 4) {
				ReadHostDWordFromLittleEndian (BX_MEM_THIS get_vector (cpu, a20addr), *(Bit32u*) data);
				return;
			}
			if (len == 2) {
				ReadHostWordFromLittleEndian (BX_MEM_THIS get_vector (cpu, a20addr), *(Bit16u*) data);
				return;
			}
			if (len == 1) {
				*(Bit8u *) data = *(BX_MEM_THIS get_vector (cpu, a20addr));
				return;
			}
		...
		if (a20addr < 0x000a0000 || a20addr >= 0x00100000) {
			// addr *not* in range 000A0000 .. 000FFFFF
			while (1) {
				*data_ptr = *(BX_MEM_THIS get_vector (cpu, a20addr));
				if (len == 1) return;
				len--;
				a20addr++;
#ifdef BX_LITTLE_ENDIAN
		...
		for (unsigned i = 0; i<len; i++) {
			...
			if (a20addr < 0x000c0000) {
				// devices are not allowed to access SMMRAM under VGA memory
				if (cpu) *data_ptr = *(BX_MEM_THIS get_vector (cpu, a20addr));
				goto inc_one;
			}
...
void BX_MEM_C::dmaWritePhysicalPage (BX_CPU_C *cpu, bx_phy_address addr, unsigned len, Bit8u *data) {
	// Note: accesses should always be contained within a single page
	if ((addr >> 12) != ((addr + len - 1) >> 12)) {
	...
	if (memptr != NULL) {
		pageWriteStampTable.decWriteStamp (cpu, addr);
